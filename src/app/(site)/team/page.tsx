...
const pageTitle = "Voquarn Code Team | Engineering, Design & Growth Leadership";
const pageDescription =
  "Meet the Voquarn Code team behind high-converting websites, apps, SEO systems, AI automation, and client delivery.";
...
export const metadata = buildMetadata(
  ...
  "/team",
  {
    keywords: [
      ...
      "SEO specialists Pakistan",
      "digital agency experts",
    ],
  },
);

// Regenerated hourly so admin edits to team members reach the live site
// without a redeploy.
export const revalidate = 3600;

export default async function TeamPage() {
  const [members, site] = await Promise.all([getTeamMembers(), getSiteSettings()]);
  ...
  return (
    <>
      ...
      <section className="page-section mt-24 lg:mt-32 pt-40 lg:pt-56">
        ...
        {/* --- PINTEREST/MUSEUM STYLE FEATURED PORTRAIT CARD --- */}
        {featured && (
            ...
            <div className="w-full p-8 sm:p-14 flex flex-col justify-between relative z-10 min-h-[300px]">
              ...
              <div className="space-y-4">
                <blockquote className="font-sans font-extralight text-2xl sm:text-[1.65rem] text-neutral-200 tracking-wide leading-relaxed">
                  {featured.bio || "Leadership requires a world view beyond oneself."}
                </blockquote>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4 text-neutral-500">
                  <a href={featured.linkedinUrl || site.socials.linkedin} target="_blank" rel="noreferrer" className="hover:text-white transition-colors" aria-label={`${featured.name} on LinkedIn`}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                    </svg>
                  </a>
                  <a href={featured.facebookUrl || site.socials.facebook} target="_blank" rel="noreferrer" className="hover:text-white transition-colors" aria-label={`${featured.name} on Facebook`}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                    </svg>
                  </a>
                  <a href={`mailto:${featured.email || site.email}`} className="hover:text-white transition-colors" aria-label={`Email ${featured.name}`}>
        ...
        <div className="mt-6 max-w-3xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {rest.map((member) => (
              <div key={member.name} className="group bg-[#111111] rounded-[2rem] p-8 border border-neutral-800 shadow-sm hover:shadow-2xl hover:border-neutral-700 hover:-translate-y-1 transition-all duration-500 relative overflow-hidden flex flex-col justify-between min-h-[220px]">
                ...
                <div className="relative z-10">
                  <h3 className="font-display font-medium text-lg text-white uppercase tracking-[0.1em] mb-1">{member.name}</h3>
                  <p className="text-[9px] font-extrabold text-neutral-500 uppercase tracking-[0.2em]">{member.role}</p>
                  {member.bio && (
                    <p className="mt-3 text-xs text-neutral-400 leading-relaxed">{member.bio}</p>
                  )}
                </div>
                <div className="relative z-10 flex items-center gap-4 mt-8">
                  <a href={member.linkedinUrl || site.socials.linkedin} target="_blank" rel="noreferrer" className="text-neutral-500 hover:text-white transition-colors" aria-label={`${member.name} on LinkedIn`}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                    </svg>
                  </a>
                  <a href={member.facebookUrl || site.socials.facebook} target="_blank" rel="noreferrer" className="text-neutral-500 hover:text-white transition-colors" aria-label={`${member.name} on Facebook`}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                    </svg>
                  </a>
                  <a href={`mailto:${member.email || site.email}`} className="text-neutral-500 hover:text-white transition-colors" aria-label={`Email ${member.name}`}>
                    <Mail className="w-5 h-5 stroke-[1.5]" />
                  </a>
                </div>
              </div>
