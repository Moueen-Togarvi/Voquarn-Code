...
import { AdminValidationError, parseClientLogo, parsePositiveId } from "@/lib/admin-validation";
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [updated] = await db.update(clientLogos).set(parseClientLogo(body)).where(eq(clientLogos.id, id)).returning();
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const id = parsePositiveId(rawId);
    const [deleted] = await db.delete(clientLogos).where(eq(clientLogos.id, id)).returning();
