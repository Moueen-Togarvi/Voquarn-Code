...
import { AdminValidationError, parsePositiveId, parseStat } from "@/lib/admin-validation";
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [updated] = await db.update(stats).set(parseStat(body)).where(eq(stats.id, id)).returning();
